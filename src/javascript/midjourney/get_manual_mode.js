...
(function() {
    ...
    function downloadCSV(csvContent, filename) {
        ...
        document.body.append(downloadLink); // Use append()
        downloadLink.click();

        // Clean up
        downloadLink.remove(); // Use remove()
    ...
    function extractParameters(jobContainer) {
        ...
        if (!paramsContainer) {
            return jobParams; // Return empty object if no parameter container found
        }
        ...
        buttons.forEach(button => {
            ...
            // Complex logic to extract value from siblings (text nodes and specific spans)
            let paramValue = '';
            ...
            // Handle parameters that are flags (exist but have no value, e.g., --style raw)
            if (paramName && !paramValue) {
                paramValue = "true"; // Assign 'true' as the value
            }
     ...
     * @param {Element} jobContainer - The DOM element representing a single job.
     * @returns {Array<object>} An array of objects, each containing { jobId, pngSrc }. Returns empty if no valid images found.
     */
    function extractImageData(jobContainer) {
        const imageData = [];
        const imageGridContainer = jobContainer.querySelector(SELECTORS.imageGrid);
        if (!imageGridContainer) return imageData; // Exit if no image grid container
        ...
        imageLinks.forEach(link => {
            ...
            const src = imgElement?.getAttribute('src'); // Use optional chaining
            if (!src) return; // Skip if no image element or src attribute
            ...
            // Convert .webp URLs to .png, handle potential errors gracefully
    ...
    function runManualExport() {
        ...
        allJobContainers.forEach((jobContainer, index) => {
            // console.debug(`${LOG_PREFIX} Analyzing Container ${index + 1}...`); // Uncomment for detailed debugging
            ...
            if (imagesData.length > 0) {
                ...
                // still record the entry with default image data. This might capture text prompts or errors.
                ...
                // Optionally log containers that yield no data if needed for debugging
                 console.debug(`${LOG_PREFIX} Container ${index + 1} yielded no usable data (prompt, params, or image). Skipping.`);
            }
        });
        ...
        intermediateData.forEach(entry => {
            ...
            const paramValues = sortedParamNames.map(colName => {
                ...
                return escapeCsvField(value ?? ''); // Use nullish coalescing for undefined/null values
            });
        ...
        if (csvRows.length > 1) { // Check if there is data beyond the header row
    ...
    // Wrap the main execution in a try...catch block to handle unexpected errors gracefully
    try {
        runManualExport();
    } catch (error) {
